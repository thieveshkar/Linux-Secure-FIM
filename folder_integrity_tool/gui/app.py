...
class FIMConsole(ctk.CTk):
    ...
    def lock_vault(self):
        ...
        for target in paths:
            self._log(f"Attempting to apply system-level vault lock (chattr +i) to {target}")
            if not os.path.exists(target):
                self._log(f"[ERROR] Target directory {target} does not exist.")
                continue

            try:
                result = subprocess.run(["sudo", "chattr", "+i", target], capture_output=True, text=True)
                if result.returncode == 0:
                    self._log(f"Vault successfully locked. Directory {target} is now immutable.")
                    messagebox.showinfo("Vault Locked", f"The directory {target} is now immutable.")
                else:
    ...
    def unlock_vault(self):
        ...
        for target in paths:
            self._log(f"Attempting to remove system-level vault lock (chattr -i) from {target}")
            if not os.path.exists(target):
                self._log(f"[ERROR] Target directory {target} does not exist.")
                continue
            ...
            try:
                result = subprocess.run(["sudo", "chattr", "-i", target], capture_output=True, text=True)
                if result.returncode == 0:
                    self._log(f"Vault successfully unlocked. Directory {target} is now mutable.")
                    messagebox.showinfo("Vault Unlocked", f"The directory {target} is now mutable.")
                else:
